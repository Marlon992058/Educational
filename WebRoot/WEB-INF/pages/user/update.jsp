				...
				<table id="add-tab" class="tm_table">
					...
					<tbody id="tmblog">
						<tr>
							...
							<td class="add-fl"><input type="text" value="${user.username}" name="username" id="username"/></td>
						</tr>
						<tr>
							<td class="add-fr">年龄：</td>
							<td class="add-fl"><input type="text" value="${user.age}" name="age" id="age"/></td>
						</tr>
						<tr>
							<td class="add-fr">性别：</td>
							<td class="add-fl" id="sex">
								<label><input type="radio" value="1" name="sex" <c:if test="${user.sex==1}">checked</c:if>/>男</label>
								<label><input type="radio" value="0" name="sex" <c:if test="${user.sex==0}">checked</c:if>/>女</label>
								<label><input type="radio" value="2" name="sex" <c:if test="${user.sex==2}">checked</c:if>/>保密</label>
							</td>
						</tr>
						<tr>
							<td class="add-fr">出生日期：</td>
							<td class="add-fl"><input type="text" value="${user.birthday}" name="birthday" id="birthday"/></td>
						</tr>
						<tr>
							<td class="add-fc" colspan="2"><a href="javascript:void(0)" onclick="tmOption.update(${user.id})">更新</a></td>
						</tr>
					</tbody>
				</table>
